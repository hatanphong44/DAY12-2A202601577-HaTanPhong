...
class Lifecycle:
    ...
    def request_shutdown(self, signum=None, frame=None) -> None:
        ...
        2. Gọi lại handler cũ (của uvicorn) để server thật sự dừng
        """
        self.shutting_down = True
        # Call the previous handler if it exists (uvicorn's handler)
    ...
    def install(self) -> None:
        ...
        for sig in (signal.SIGTERM, signal.SIGINT):
            self._previous[sig] = signal.getsignal(sig)  # Remember the old handler
            signal.signal(sig, self.request_shutdown)    # Override with ours
